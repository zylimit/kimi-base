...
P11 批次二完成：派单第七字段与回流通道（REQ-070）、交互深度四档（REQ-071）、skill 工艺补齐与去重（REQ-075）落地并获资产锚点测试锁定（tests/spec.test.mjs「资产锚点：内容面」8 用例），按 REQ-067 机制摘 planned 标记。REQ-072/073/074 保持 planned——其验收含 P11 收尾的真实情境前后对比检验，检验完成前不摘。

### 变更

- REQ-070/071/075 摘 planned(P11) 标记。
- 版本号 v3.1.1 → v3.1.2。

## v3.1.1（2026-09-08）P6 激活：REQ-058 feedback 引擎化

### 为什么改

P6 完成，按 REQ-067 机制摘除 planned 标记。feedback 四层进化从纯提示词纪律（索引曾残缺虚报待办）落成机器维护面：record/list/scan/propose 四动词（ADR-0010），索引由引擎单一维护，毕业落地恒需人工确认。

### 变更

- REQ-058（feedback 引擎化）摘 planned(P6) 标记，验收 tests/feedback.test.mjs 20 用例全绿（红测先行，实现未动任何测试文件与断言）；tests/cli-contracts.test.mjs 扩表（39+help → 40+help）同批转绿。
- 版本号 v3.1.0 → v3.1.1。

## v3.1.0（2026-09-08）沟通桥梁层立项（REQ-069~075）+ P4 激活

### 为什么改

用户从产品本质提出：框架是人与 AI 共同理解问题、形成判断、完成交付的沟通桥梁，业务理解与工程执行必须同样强大。软层审计（16 skills/8 agents/13 模板逐行取证）发现七大断点（认知无标注/业务含义不过链/交互深度一刀切/迭代不采动机/界面阶段缺失/记忆无依据链/无反例回流），外部实证（knowing-but-not-showing、Lost-in-Conversation、ReqElicitGym）证明不能靠模型自觉。设计取证全文在 docs/COMMUNICATION-LAYER.md，决策在 ADR-0012。
