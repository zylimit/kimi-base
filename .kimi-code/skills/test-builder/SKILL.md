...
从 Spec / 派单包 Business Context 提取断言时，若发现 Spec 本身可疑（业务上说不通、与已确认场景冲突、动机与受益人对不上），**不写猜测性断言替需求做发明**：产出「需求存疑」条目——**哪条 REQ / 为什么可疑 / 证据**——写入回执 `Needs review by` 并显式标注「回流 product-spec-builder 迭代模式」，由主 Agent 路由；该锚点的用例暂缓，标进覆盖缺口并注明原因。确认的 Spec 错误修正后，反例进 Spec「规则与例外」节（无此节则新增），新增用例锚定该例外条目。
