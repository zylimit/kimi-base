...
```text
node .kimi-base/runtime/kimi-base.mjs catalog discover           # 绿地/棕地：从仓库事实推导 catalog 草案（dry-run）
node .kimi-base/runtime/kimi-base.mjs catalog discover --write   # 落盘（已有 catalog 时写 module-catalog.draft.json）
#   然后人逐项决定 needsDecision（属性档位/forbiddenDependencies/层名/矩阵接线）——草案是提案不是决定
node .kimi-base/runtime/kimi-base.mjs catalog lint               # 校验：每条 tracked path 必须有归处
# init-modules 是 catalog discover 的废弃别名（legacy，转发并注明）
```
