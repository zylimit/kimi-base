...
2. 缺工具/缺命令/非 git 仓 = BLOCKED 或可见降级，绝不假绿；SKIP 必须显式——`gate` 四态门机械输出。
3. security/safety/privacy 永不豁免、永不 fast-skip；`waiver` 只豁免"跑不了"，不豁免"跑挂了"（配置期拒绝 protected 豁免）。
4. 声明任何东西可用前：说出能证明它的命令 → 现跑 → 读完整输出和退出码 → 确认输出支撑该结论 → 才准说（提示词纪律）。

## 工程纪律

1. 测试独立：写测者 ≠ 被测作者（提示词纪律）；测试不为迁就实现改断言。行为测试在临时 git 仓中跑，断言退出码与 JSON 字段，不断言 stderr 文本。
...
4. 引擎改动保持退出码契约：0 干净 / 1 用法·违规 / 2 阻断 / 3 降级 / 4 陈旧——行为测试逐 verb 断言（`unit` 检查锁死）。
5. 三文件同步：治理代码与 progress.md 同 commit；Product-Spec.md 改动与其 CHANGELOG 同 commit（`sync-check` 机械执法）。
