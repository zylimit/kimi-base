...
复杂、跨模块或中高风险任务先建立 task（JSON 传入 envelope、risk、ownedPaths、Spec/Plan 引用和 review exclusions）：

```text
node .kimi-base/runtime/kimi-base.mjs task start --json <task.json>
...
Fast Mode（`node .kimi-base/runtime/kimi-base.mjs fast on|off|status`）下不自动新增测试或派 reviewer/tester，但用户显式要求的检查、安全门和不可跳过 check 仍执行；security/safety 永不 fast-skip。
