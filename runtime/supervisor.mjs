...
function projectRoot(flags) {
  ...
  if (!root) die(`未找到 .kimi-base/harness.json（自 ${start} 向上）；supervisor 只在 kimi-base 项目内工作`, 3);
  return root;
}
...
function cmdStart(flags, name) {
  if (!name) die('start 需要服务名：supervisor.mjs start <name>', 3);
...
function cmdStop(flags, name) {
  if (!name) die('stop 需要服务名：supervisor.mjs stop <name>', 3);
  const root = projectRoot(flags);
  const current = readState(root, name);
...
function cmdLogs(flags, name) {
  if (!name) die('logs 需要服务名：supervisor.mjs logs <name> [--lines 80]', 3);
  const root = projectRoot(flags);
